...
class PIDController(object):
    ...
    def control(self, target, sensor):
        ...
        error = target - sensor
        deltaU = (
            (self.Kp + self.Ki *self.dt + (self.Kd/self.dt))
            * error -(self.Kp + (2*self.Kd/self.dt))* self.e1
            + (self.Kd/self.dt)*self.e2
        )
        self.u = self.u + deltaU
        e1 = self.e1
        self.e2 = e1
        self.e1 = error
        predictSensor = sensor +self.u* self.dt
        self.y.append(predictSensor)
        return self.u
